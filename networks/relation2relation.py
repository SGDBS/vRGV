...
class AttHierarchicalGround(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, visual_dim, word_dim, num_layers=1):
        ...
        encoderLayer = nn.TransformerEncoderLayer(d_model=self.embed_dim, nhead=4, dropout=0.3, dim_feedforward=256)
        self.feature_transformer = nn.TransformerEncoder(encoder_layer=encoderLayer, num_layers=1)
    ...
    def feature_summary(self, video_embed, obj, sub):
        obj_expand = obj.unsqueeze(2)
        sub_expand = sub.unsqueeze(2)
        obj_main = video_embed * obj_expand
        sub_main = video_embed * sub_expand
        obj_main = torch.sum(obj_main, dim=1)
        sub_main = torch.sum(sub_main, dim=1) # [120, 256]
        obj_main = self.feature_transformer(obj_main)
        sub_main = self.feature_transformer(sub_main)
        return obj_main, sub_main # [120, 256]
    ...
    def spatialAtt(self, videos, relations):
        ...
        for bs in range(batch_size):
            ...
            obj_main, sub_main = self.feature_summary(video_embed, obj_att, sub_att)

            final_subject_feat = subject_feat + sub_main + o2s_feat
            final_object_feat = object_feat + obj_main + s2o_feat
    ...
    def forward(self, videos, relation_text, mode='train'):
        ...
        #############################################################
        # add a linear layer after the video global feature to better fit the clr loss
        # output = self.video_out_linear(output)
        ############################################################
