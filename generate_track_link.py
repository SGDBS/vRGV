...
def generate_track(val_list_file, results_file, feat_dir, bbox_dir, res_file):
    ...
    sample_frames = None
    results_file = "/home/gpu4/Vigelos/ground_data/results/vidvrd_batch.json"
    results_all = load_file(results_file)
    print("load result from results_file:",results_file)

    final_res = {}
    video_res = {}

    final_res_without_viterbi = {}
    video_res_without_viterbi = {}
    ...
    for i, sample in enumerate(val_list):
        ...
        #t1 = time.time()
        sub_bboxes, obj_bboxes, sid, valid_frame_idx, without_viterbi = link_bbox(video_bboxes, alpha_s, alpha_o,
        ...
        video_res_without_viterbi[relation] = without_viterbi
        # print("video res:", video_res.keys())
        # print("video res without viterbi:", video_res_without_viterbi.keys())
        ...
        if i == total_n -1:
            ...
            final_res_without_viterbi[vname] = video_res_without_viterbi


    save_results(res_file, final_res)
    save_results(res_file.replace('.json', '_without_viterbi.json'), final_res_without_viterbi)
    print("result without viterbi saved")
...
def main(res_file):
    ...
    print("generating track link with beta threshold:{}".format(beta_thresh))

    data_dir = '/home/gpu4/Vigelos/vRGV_data/ground_data'
    dataset = 'vidvrd'
    ...
    #feat_dir = osp.join(data_dir, dataset, 'video_feature')
