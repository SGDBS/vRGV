...
gt_frame_dict = {}
with open("dataset/vidvrd/vrelation_val.json") as f:
    data = json.load(f)
    for item in data:
        vname = item[0]
        frame_count = item[1]
        if vname not in gt_frame_dict:
            gt_frame_dict[vname] = frame_count

beta_dict = {}
with open("../ground_data/results/vidvrd_batch.json") as f:
    data = json.load(f)
    for vname in data.keys():
        if vname not in beta_dict:
            beta_dict[vname] = {}

        for relation_text in data[vname].keys():
            b1 = data[vname][relation_text]['beta1']
            b2 = data[vname][relation_text]['beta2']
            b = b1
            for i in range(120):
                b[i] += b2[i//12]
            beta_dict[vname][relation_text] = b


def eval_ground_scores(gt_relations, pred_relations, tiou_threshold,qid):
    """
    :param gt_relations:
    ...
    gt_start, gt_end, pred_start, pred_end = 0,0,0,0
    ...
    for relation, pred_trajs in pred_relations.items():
        ...
        # gt_length.append(round(len(gt_trajs[0]['sub']) / gt_frame_dict[qid] * 120))
        # print(round(len(gt_trajs[0]['sub']) / gt_frame_dict[qid] * 120))


        gt_start = round(int(list(gt_trajs[0]['obj'].keys())[0])/gt_frame_dict[qid]*120)
        gt_end = round(int(list(gt_trajs[0]['obj'].keys())[-1])/gt_frame_dict[qid]*120)

        if(pred_obj=={}):
            pred_start = 0
            pred_end = 0
        else:
            pred_start = round(int(list(pred_obj.keys())[0])/gt_frame_dict[qid]*120)
            pred_end = round(int(list(pred_obj.keys())[-1])/gt_frame_dict[qid]*120)
        # print(gt_start,gt_end,pred_start,pred_end)


        info_item = beta_dict[qid][relation]
        info_item.append(gt_start)
        info_item.append(gt_end)
        info_item.append(pred_start)
        info_item.append(pred_end)
...
def evaluate(groundtruth, prediction, tiou_threshold=0.5):
    ...
    for qid, relation_gt in groundtruth.items():
        ...
        video_acc_reverse, video_acc_sub_reverse, video_acc_obj_reverse, video_acc_spatial, video_acc_temporal, video_bd_motions_all, video_bd_motions_correct, video_acc, video_acc_sub, video_acc_obj, video_acc_static, video_acc_sub_static, video_acc_obj_static, video_acc_dynamic, video_acc_sub_dynamic, video_acc_obj_dynamic, video_acc_zero_shot, video_acc_sub_zero_shot, video_acc_obj_zero_shot =  eval_ground_scores(relation_gt, relation_pred, tiou_threshold,qid)
...
def main():
    ...
    with open('attn_info.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerows(info)

    # with open("dataset/vidvrd/gt_relation_frame.json") as f:
    #     data = json.load(f)
    # print(data['ILSVRC2015_train_00010001']['bicycle-move_beneath-person'][0]['sub'].keys())
    # print(data['ILSVRC2015_train_00010001']['bicycle-move_beneath-person'][1]['sub'].keys())
