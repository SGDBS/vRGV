...
def temporal_ground(temp, nframe, t_thresh):
    ...
    # print('sample_nframe:', sample_nframe)
    # print('nframe:', nframe)
...
def build_link(bboxes, scores_sub, scores_obj, clip, sample_frames):
    ...
    :return:
    ########################
    # when factor is set to zero, it is the * mode in the original paper
    ########################
    """
...
def get_tube(bboxes, alpha_s, alpha_o, clip, sample_frames):
    ...
    # print("valid frame num: ", valid_frame_idx," len:",len(valid_frame_idx))

    alpha_s_hardmax = np.argmax(alpha_s,axis=1)[valid_frame_idx]
    alpha_o_hardmax = np.argmax(alpha_o,axis=1)[valid_frame_idx]
    hardmax_s = {}
    for i in range(cbboxes.shape[0]):
        hardmax_s[valid_frame_idx[i]] = cbboxes[i][alpha_s_hardmax[i]].tolist()
    hardmax_o = {}
    for i in range(cbboxes.shape[0]):
        hardmax_o[valid_frame_idx[i]] = cbboxes[i][alpha_o_hardmax[i]].tolist()
    without_viterbi = {"sub":hardmax_s,"obj":hardmax_o}
    ...
    return sub_tube, max_score_sub, obj_tube, max_score_obj,valid_frame_idx,without_viterbi
...
def link_bbox(video_bboxes, alpha_s, alpha_o, temp, beta_thresh, sample_frames, nframe):
    ...
    without_viterbi = None

    for clip in clips:
        sub, sub_score, obj, obj_score, clip_frame_idx, without_viterbi = get_tube(video_bboxes, alpha_s, alpha_o, clip, sample_frames)
    ...
    return T_s, T_o, sid, valid_frame_idx, without_viterbi
